...
for file in $(ls -1 /etc/cron.d/); do
    (cat /etc/cron.d/$file; echo) | while read -r line; do
		...
		set -- $line
		cron_command=$(echo "${@:7:$#}")
	        cron_command=$(echo "$cron_command" | sed -e 's,\\,\\\\,g' -e 's,*,\\*,g' -e 's,=,\\=,g')
		line_replace=$(echo "$line" | sed -e "s,$cron_command,$insert_script \"&\",")
		line_replace=$(echo "$line_replace" | sed -e 's,\\,\\\\,g' -e 's,*,\\*,g'  -e 's,=,\\=,g' -e 's,&,\\&,g' -e 's,\,,\\\,,g')
		line_old=$(echo "$line" | sed -e 's,\\,\\\\,g' -e 's,*,\\*,g'  -e 's,=,\\=,g' -e 's,\,,\\\,,g')
		sed -i "s,$line_old,$line_replace," /etc/cron.d/$file
